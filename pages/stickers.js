...
const srtickers = ({ products }) => {
  console.log("hoods", products);
  return (
    <div>
       <section className="text-gray-600 body-font">
  <div className="container px-5 py-5 mx-auto">
    <div className="flex flex-wrap m-4">
      {Object.keys(products).length === 0 && <p>Sorry all hoods are currently out of stocks!, New stock coming soon. Stay tuned.</p>}
           {Object.keys(products).map((item)=>{ 
                return <div key={products[item]._id} className="lg:w-1/4 md:w-1/2 w-full p-2 shadow-lg">
          <a href={`/product/${products[item].slug}`} className="block rounded ">
...
export async function getServerSideProps(context) {
  ...
  let products = await Product.find({category:'srtickers'});
  let hoods = {}
